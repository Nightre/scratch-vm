...
class ScriptTreeGenerator {
    constructor(thread, functionData) {
        /** @private */
        this.thread = thread;
        /** @private */
         ...
         */
        this.script = new IntermediateScript(functionData);
    ...
    setProcedureVariant(procedureVariant, independentThread) {
    ...
    descendExtensible(block) {
        const size = parseInt(block.mutation.size)
        const res = []
        for (let index = 0; index < size; index++) {
            res.push(this.descendInputOfBlock(block, ExtensibleKey + index))
        }
        return res
    }
    /**
    ...
    descendInput(block) {
        switch (block.opcode) {
            ...
            case 'structures_self':
                return {
                    kind: "structures.self"
                }
            case 'structures_create_list':
                return {
                    kind: 'structures.create_list',
                    extensible: this.descendExtensible(block)
                };
            case 'structures_create_json':
                return {
                    kind: 'structures.create_map',
                    extensible: this.descendExtensible(block)
                };
            case 'structures_get_attribute':
                return {
                    kind: 'structures.get_attribute',
                    extensible: this.descendExtensible(block),
    ...
    _descendVariable(id, name, type) {
        ...
        // Create it locally...
        const newVariable = new Variable(id, name, type, false, target);
        ...
        if (target.sprite) {
            ...
            for (const clone of target.sprite.clones) {
                if (!Object.prototype.hasOwnProperty.call(clone.variables, id)) {
                    clone.variables[id] = new Variable(id, name, type, false, target);
                }
            }
        }
...
class IRGenerator {
    ...
    generate(functionData) {
        const mainGenerator = new ScriptTreeGenerator(this.thread, functionData)
        if (functionData) {
            // 直接触发函数时使用
            mainGenerator.setProcedureVariant(generateProcedureVariant(functionData.code, false), true)
            if (functionData.isWarp) mainGenerator.enableWarp()
        }

        const entry = this.generateScriptTree(mainGenerator, this.thread.topBlock);
