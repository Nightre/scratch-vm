    ...
    descendInput(node) {
        switch (node.kind) {
            ...
            case 'structures.self':
                return new TypedInput("target.getData()", TYPE_UNKNOWN);
            case 'structures.create_list':
                let code = '['
                node.extensible.forEach((input, index) => {
                    code += this.descendInput(input).asUnknown()
                    if (index < node.extensible.length - 1) {
                        code += ', ';
                    }
                });
                code += ']'
            ...
            case 'structures.create_map':
                let mapCode = '{';

                for (let i = 0; i < node.extensible.length; i += 2) {
                    const keyInput = node.extensible[i];
                    const valueInput = node.extensible[i + 1];

                    const key = this.descendInput(keyInput).asUnknown();
                    const value = this.descendInput(valueInput).asUnknown();

                    mapCode += `${key}:${value}`;

                    if (i < node.extensible.length - 2) {
                        mapCode += ', ';
                    }
                }
